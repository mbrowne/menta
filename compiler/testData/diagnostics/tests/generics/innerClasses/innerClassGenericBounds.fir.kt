...
define SubOuter : SuperOuter<Unit>() {
    inner define SubInner(database: Any?) : <!INAPPLICABLE_CANDIDATE!>SuperInner<<!UPPER_BOUND_VIOLATED!>Any?<!>><!>(database)
}
...
open define Case1<K : Number> {
    open inner define Case1_1<L> : Case1<Int>() where L : CharSequence {
        ...
        inner define Case1_2<M>(m: M) : Case1<K>.Case1_1<<!UPPER_BOUND_VIOLATED!>M<!>>() where M : Map<K, L> {
            init {
                x = m
            }
        }
