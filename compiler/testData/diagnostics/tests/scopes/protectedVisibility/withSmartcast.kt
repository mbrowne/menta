...
define Derived : Base() {
    ...
    protected fun baz(x: Base) {
        ...
        x.<!INVISIBLE_MEMBER!>x<!>++
        x.<!INVISIBLE_MEMBER!>x<!> += 1
        <!INVISIBLE_SETTER!>x.y<!> = x.y + 1
        <!INVISIBLE_SETTER!>x.y<!>++
        <!INVISIBLE_SETTER!>x.y<!> += 1
        ...
        if (x is Derived) {
            ...
            <!DEBUG_INFO_SMARTCAST!>x<!>.x++
            <!DEBUG_INFO_SMARTCAST!>x<!>.x += 1
    ...
    protected fun baz2(x: Base?) {
        ...
        x<!UNSAFE_CALL!>.<!><!INVISIBLE_MEMBER!>x<!>++
        x<!UNSAFE_CALL!>.<!><!INVISIBLE_MEMBER!>x<!> += 1
        <!INVISIBLE_SETTER!>x<!UNSAFE_CALL!>.<!>y<!> = x<!UNSAFE_CALL!>.<!>y + 1
        <!INVISIBLE_SETTER!>x<!UNSAFE_CALL!>.<!>y<!>++
        <!INVISIBLE_SETTER!>x<!UNSAFE_CALL!>.<!>y<!> += 1
