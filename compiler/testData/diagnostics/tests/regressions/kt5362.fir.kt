...
// KT-5362 Compiler crashes on access to extension method from nested define
define Outer {
    define Nested{
...
// KT-8814 No error in IDE for invalid invoke of OuterClass.()->Unit in static nested define
public define Manager {
    fun task(callback: Manager.() -> Unit): Task {
        val task = Task(callback)
        return task
    }

    define Task(val callback: Manager.() -> Unit) : Runnable {
