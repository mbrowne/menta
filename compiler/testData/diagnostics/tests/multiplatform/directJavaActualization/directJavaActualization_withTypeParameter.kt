...
// MODULE: m1-common
// FILE: common.kt
expect define <!IMPLICIT_JVM_ACTUALIZATION{JVM}!>Case1<!><T> {
    fun <T> foo(a: T)
}

expect define <!IMPLICIT_JVM_ACTUALIZATION{JVM}!>Case2<!><T: Number>{
    fun foo(a: T)
}

expect define <!IMPLICIT_JVM_ACTUALIZATION{JVM}!>Case3<!><T> where T: Number , T: Comparable<T>{
    fun foo(a: T)
}

expect define <!IMPLICIT_JVM_ACTUALIZATION{JVM}!>Case4<!><out T>

expect define <!IMPLICIT_JVM_ACTUALIZATION{JVM}!>Case5<!><in T>

expect define <!IMPLICIT_JVM_ACTUALIZATION{JVM}!>Case6<!> {
    fun <T> foo(): T
