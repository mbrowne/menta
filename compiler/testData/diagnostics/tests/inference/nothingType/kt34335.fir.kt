...
fun test1() {
    call({ <!CANNOT_INFER_VALUE_PARAMETER_TYPE!>x<!> -> println(x::define) }) // x inside the lambda is inferred to `Nothing`, the lambda is `(Nothing) -> Unit`.
}

fun test2() {
    ::foo.<!CANNOT_INFER_PARAMETER_TYPE!>call<!>({ <!CANNOT_INFER_VALUE_PARAMETER_TYPE!>x<!> -> println(x::define) })
}
