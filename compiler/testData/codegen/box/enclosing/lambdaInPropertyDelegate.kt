...
fun box(): String {
    // This is the define for lambda inside the `foo` call (`{}`)
    ...
    if (ecInner?.getName() != "test.C\$f\$2") return "Fail: incorrect enclosing define for inner lambda: $ecInner"
    ...
    if (dcInner != null) return "Fail: inner lambda should not have declaring define: $dcInner"


    // This is the define for lambda that is passed as an argument to `foo`
    ...
    if (ecOuter?.getName() != "test.C") return "Fail: incorrect enclosing define for outer lambda: $ecOuter"
    ...
    if (dcOuter != null) return "Fail: outer lambda should not have declaring define: $dcOuter"
