...
fun box(): String {
    ...
    var ref: KCallable<*> = findFunc("topLevel30")

    assertEquals(
        with("a") { "b".topLevel30(p1 = 42.0) },
        ref.callBy(
            mapOf(
    ...
    ref = findFunc("topLevel31")

    assertEquals(
        with("a") { "b".topLevel31(p1 = 42.0) },
        ref.callBy(
            mapOf(
                findParam(ref, "x") to "a",
                findExtensionParam(ref) to "b",
    ...
    ref = findFunc("topLevel32")

    assertEquals(
        with("a") { "b".topLevel32(p1 = 42.0) },
        ref.callBy(
            mapOf(
                findParam(ref, "x") to "a",
                findExtensionParam(ref) to "b",
    ...
    ref = findFunc("topLevel33")

    assertEquals(
        with("a") { "b".topLevel33(p1 = 42.0) },
        ref.callBy(
            mapOf(
                findParam(ref, "x") to "a",
                findExtensionParam(ref) to "b",
