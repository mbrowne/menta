    ...
    target.defFiles().forEach { df ->
        ...
        val libTask = tasks.register(interopTaskName(libName, targetName), KonanInteropTask::class.java) {
            ...
            if (kotlinBuildProperties.buildPlatformLibsByBootstrapCompiler) {
                ...
                dependsOn(":menta-native:distCompiler")
                dependsOn(":menta-native:distStdlib")
            }
        ...
        val klibInstallTask = tasks.register(libName, Sync::class.java) {
            // During the execution of the `:menta-native:publish` task, the `:menta-native:bundleRegular` subtask
            ...
            // `bundlePrebuilt` subtask, which is also triggered by calling `:menta-native:publish`.
            //
            ...
            mustRunAfter(":menta-native:bundleRegular")
